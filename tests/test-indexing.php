...
class Tests_Indexing extends SearchPress_UnitTestCase {
	...
	public function test_post_statuses( $status, $index, $search, $cs_args = false ) {
		...
		$results = $this->search_and_get_field(
			array(
				'query'       => 'test post',
				'post_status' => $status,
			)
	...
	public function test_post_types( $type, $index, $search, $cpt_args = false ) {
		...
		if ( 'attachment' === $type ) {
			...
			$post_id   = $this->factory->attachment->create_object(
				'image.jpg', $parent_id, array(
					'post_mime_type' => 'image/jpeg',
					'post_type'      => 'attachment',
					'post_title'     => 'test post',
				)
	...
	function test_cron_index_non_200() {
		...
		SP_Config()->update_settings(
			array(
				'host'   => 'http://asdftestblog1.files.wordpress.com',
				'active' => false,
			)
		);
	...
	public function test_invalid_data() {
		...
		// format.
		add_filter(
			'sp_post_pre_index', function( $data ) {
